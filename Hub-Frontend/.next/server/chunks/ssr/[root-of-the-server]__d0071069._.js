...
function PostsPage() {
    ...
    const [resetUploader, setResetUploader] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useState"])(0);
    ...
    // Handle file upload completion when manually uploading files
    const handleUploadComplete = (fileUrls)=>{
        console.log("Files uploaded manually:", fileUrls);
        setMediaLinks((prevLinks)=>{
            // Add only new URLs that don't already exist
            const newLinks = fileUrls.filter((url)=>!prevLinks.includes(url));
            return [
                ...prevLinks,
                ...newLinks
            ];
    ...
    // Handle post creation with S3 integration
    const handleCreatePost = async ()=>{
        ...
        try {
            setIsUploading(true);
            // First upload all files to S3 if there are any pending uploads
            let uploadedUrls = [];
            if (fileUploaderRef.current) {
                try {
                    uploadedUrls = await fileUploaderRef.current.uploadAllFiles();
            ...
            // Combine any previously uploaded files with new uploads
            const allMediaLinks = [
                ...mediaLinks,
                ...uploadedUrls.filter((url)=>!mediaLinks.includes(url))
            ];
            ...
            // Create the post with all media links
...
//# sourceMappingURL=%5Broot-of-the-server%5D__d0071069._.js.map